# 🦾 Aria: J.A.R.V.I.S. Protocol

<div align="center">

[![System Status](https://img.shields.io/badge/System-ONLINE-success?style=flat-square)](https://github.com/Zephyr-Wach/AIGF-Aria)
[![Architecture](https://img.shields.io/badge/Architecture-Hybrid_(Local_%2B_Cloud)-blueviolet?style=flat-square)](https://github.com/ml-explore/mlx)
[![Class](https://img.shields.io/badge/Class-Cognitive_Daemon-orange?style=flat-square)](LICENSE)

**Just A Rather Intelligent Agent (只是一个相当智能的代理)**
*你的通用型认知操作系统。*

[English](./docs/README_EN.md) | [简体中文](README.md)

</div>

---

## 📡 项目现状 (Project Status) 

**Aria 目前主要作为一个“人格修正补丁” (LoRA) 存在。** 她是一个经过深度微调的模型权重，旨在将 Qwen的通用能力转化为一个具有**批判性思维、极客审美与逻辑共鸣**的“智性伴侣”。 本项目 (`AIGF-Aria`) 的长期目标是围绕这个人格核心，构建一套类似 J.A.R.V.I.S. 的本地代理框架。**目前，LoRA 权重已稳定可用（Soul），而代理工具链（Body）正在积极开发中。**

> "灵魂已就位，正在构建躯壳。"

---

## 🗺️ 开发路线图 (Development Roadmap)

### Phase 1: 灵魂注入 (The Foundation)
*确立人格基调。*

- [x] **Milestone 1.1: 环境与基础回路**
    - [x] Task: 配置环境与依赖。
    - [x] Task: 实现基于本地模型的基础循环。
    - [x] Task: 定义基础 System Prompt 结构。

### Phase 2: 感官延伸 (The Tools)
*将 Aria 连接至本地文件系统与工作流、长期记忆。*

- [x] **Milestone 2.1: 指令路由器**
    - [x] Task: 重构主循环以支持 `/command` 指令解析。
    - [x] Task: 创建抽象 `Tool` 接口类。
- [ ] **Milestone 2.2: 记忆持久化**
    - [ ] Task: 实现 `save_history()` 和 `load_history()` (JSON/DB)。
    - [ ] Task: 实现上下文窗口管理 (保留最近 N 轮)。
    - [ ] **Challenge:** 实现系统重启后的意识连续性。

### Phase 3: 赛博共生 (Autonomy)
*赋予 Aria 主动性与视觉能力。*

- [ ] **Milestone 3.1: 主动触发器**
    - [ ] Task: 实现空闲检测 (无操作 30 分钟后触发)。
    - [ ] Task: 实现基于时间的问候 (早安/晚安)。
- [ ] **Milestone 3.2: 视觉皮层**
    - [ ] Task: 集成视觉模型 (如 Qwen-VL)。
    - [ ] Task: 创建 `/look` 指令以分析屏幕内容。

---

## 📈 版本迭代记录 (Changelog)

| 版本     | 核心困境                           | 解决方案与改进                                               | 状态         |
| :------- | :--------------------------------- | :----------------------------------------------------------- | :----------- |
| **v1.0** | 复读机效应，对话无逻辑             | 建立 JSONL 结构化数据处理流程                                | ❌ 废弃       |
| **v2.0** | 剧情炸弹: 频繁提到撞车/考试等琐事  | 引入 `device_map` 适配，尝试初步清洗                         | ⚠️ 停用       |
| **v3.1** | 过度洗脑: 5 Epoch 导致模型丧失逻辑 | 降低训练强度，反思 Rank 权重                                 | ⚠️ 停用       |
| **v3.2** | 神志清醒: 语感与逻辑的平衡点       | **物理黑名单过滤 + 极低 Rank (8)**                           | ✅ **Stable** |
| **v4.1** | v3.2基础上从3B提升到7/8B           | 量化 7B 模型便于 MacBook Air 使用                            | ✅ **Stable** |
| **v4.2** | aria理解用户输入语句执行指令       | [**语义路由 (Semantic Router)** + **JIT 动态注入** + **ReAct 回旋镖架构**](./docs/Aria_v4.2_Architecture.md) | ✅ **Stable** |
| **v4.3** | 对于长期记忆的处理                 | [三层阶梯式存储架构](./docs/Aria_v4.3_Memory_System.md)      |              |

---

## 📥 模型权重下载 (Model Weights)

Aria 的 LoRA 权重已托管至 Hugging Face。该权重基于 Qwen2.5-3B-Instruct 训练，专门优化了日常对话的语感与情感表达。

| 模型版本               | 底座模型                                                     | LoRA                                                         |
| :--------------------- | :----------------------------------------------------------- | :----------------------------------------------------------- |
| **Aria-v3.2 (稳定版)** | [Qwen2.5-3B-Instruct](https://huggingface.co/Qwen/Qwen2.5-3B-Instruct) | [zephyr-zw666/AIGF-Aria-v3.2-LoRA](https://huggingface.co/zephyr-zw666/AIGF-Aria-v3.2-LoRA) |
| **Aria-v4.0 (稳定版)** | [zephyr-zw666/Qwen2.5-7B-4bit](https://huggingface.co/zephyr-zw666/Qwen2.5-7B-4bit) | [zephyr-zw666/AIGF-Aria-v4.0-LoRA](https://huggingface.co/zephyr-zw666/AIGF-Aria-v4.0-LoRA) |

---

## 🛠️ 复刻指南 (Replication Protocol)

如果你想训练属于自己的 Aria，可以参考本仓库的文件结构与流程。

### 1. 目录结构 (Directory Structure)

```text
TrainAIGF/
├── README.md                  # 项目说明文档
├── environment.yml            # Conda 环境配置文件
├── AIGF-Aria-LoRA/            # 存放训练好的 LoRA 权重
│   ├── AIGF-Aria-v3.2-LoRA
│   └── AIGF-Aria-v4.0-LoRA
├── DataSet/                   # 原始数据集与构建脚本
│   ├── WeChatMsg_example.txt  # [示例] 聊天记录格式
│   └── build_dataset_V3.2.py  # 数据清洗脚本
├── chat_lora/                 # 推理/对话脚本
│   ├── chat_v3.2.1.py
│   └── chat_v4.1.0.py
├── data/                      # 训练用的格式化数据 (.jsonl)
│   └── train_v3.2_example.jsonl
├── models/                    # 基座模型 (Base Models)
│   ├── Qwen2.5-3B-Instruct
│   └── Qwen2.5-7B-4bit
└── train_lora/                # 训练/微调脚本
    ├── train_v3.2.py
    ├── train_v4.0_14b.py
    └── train_v4.0_7b.py
```

### 2. 详细步骤 (Step-by-Step)

1. **准备数据:**

   - 下载你和目标的聊天记录。请参考 `DataSet/WeChatMsg_example.txt` 的格式进行整理。

2. **数据清洗与处理:**

   - 运行 `DataSet/build_dataset_V3.2.py`。
   - **⚠️ 关键警告:** 代码内设置了黑名单机制。请务必修改并屏蔽高频的特殊词汇或个人隐私。否则 LoRA 会“过拟合”这些词汇，导致 Aria 后续只会背诵聊天记录，失去泛化能力。

3. **准备基座模型 (Base Model):**

   - **3B 模型:** 如果你的环境与我相同 (MacBook Air)，推荐下载 `Qwen2.5-3B-Instruct` 进行训练。

   - **7B/8B 模型警告:**

     > MacBook Air (16GB) 无法本地训练 7B/8B 模型，会发生显存溢出 (OOM)。 我是在云算力平台训练后，将权重拉取到本地的。 本地运行 7B/8B 底座必须先进行 **4-bit 量化**。

   - **14B 模型警告:** 不建议尝试。Qwen2.5-14B 的 Thinking Process 会导致模型陷入无限思考循环，不适合日常聊天交互。

4. **配置训练环境:**

   - 修改 `train_lora` 目录下的脚本文件路径，确保指向你本地的真实路径。

5. **启动与装载:**

   - 使用训练完成的 LoRA 权重配合底座模型运行 `chat_lora` 脚本即可唤醒 Aria。